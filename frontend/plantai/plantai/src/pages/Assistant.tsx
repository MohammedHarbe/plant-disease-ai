...
export default function Assistant() {
  ...
  const send = async (value = text) => {
    ...
    setBusy(true);
    try {
      const reply = await askPlantAI(value);
  ...
  return <div className="p-5 lg:p-9">
    ...
    <div className="mx-auto max-w-5xl"><div className="card overflow-hidden">
      <div className="flex items-center gap-3 border-b border-[#e8eee5] bg-[#fbfcf9] p-5"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-[#dcefd4] text-[#35633f]"><Bot size={21} /></div><div><div className="font-display font-extrabold">PlantAI Assistant</div><div className="flex items-center gap-1.5 text-xs text-[#738177]"><span className="h-2 w-2 rounded-full bg-[#6fae68]" />Online · context: Tomato Early Blight</div></div><button onClick={() => setMessages([])} className="ml-auto grid h-10 w-10 place-items-center rounded-xl text-[#879189] hover:bg-white" title="Clear chat"><RotateCcw size={16} /></button></div>
