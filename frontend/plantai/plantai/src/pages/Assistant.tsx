...
const ASSISTANT_MESSAGES_KEY='plantai:assistantMessages';

function readStoredMessages():Msg[]|null{
  try{
    const raw=sessionStorage.getItem(ASSISTANT_MESSAGES_KEY);
    if(!raw)return null;
    const parsed=JSON.parse(raw);
    if(!Array.isArray(parsed))return null;
    const messages=parsed.filter((message):message is Msg=>
      message
...
export default function Assistant() {
  ...
  const [messages, setMessages] = useState<Msg[]>(()=>readStoredMessages()||[{role: 'assistant', text: initialGreeting}]);
  ...
  useEffect(()=>{
    if(messages.length){
      sessionStorage.setItem(ASSISTANT_MESSAGES_KEY,JSON.stringify(messages));
      return;
    }
    sessionStorage.removeItem(ASSISTANT_MESSAGES_KEY);
  },[messages]);
  ...
  useEffect(()=>{
    ...
    setSlowResponse(false);
    const timer=window.setTimeout(()=>setSlowResponse(true),2500);
  ...
  const loadingText=slowResponse?'Still working on your plant analysis...':'PlantAI is thinking...';
  const clearChat=()=>{
    sessionStorage.removeItem(ASSISTANT_MESSAGES_KEY);
    setMessages([]);
  };
  ...
  return <div className="p-5 lg:p-9">
    ...
    <div className="mx-auto max-w-5xl"><div className="card overflow-hidden">
      <div className="flex items-center gap-3 border-b border-[#e8eee5] bg-[#fbfcf9] p-5"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-[#dcefd4] text-[#35633f]"><Bot size={21} /></div><div><div className="font-display font-extrabold">PlantAI Assistant</div><div className="flex items-center gap-1.5 text-xs text-[#738177]"><span className="h-2 w-2 rounded-full bg-[#6fae68]" />Online · context: {String(analysisContext?.plant || 'general plant care')}{analysisContext?.disease ? ` · ${String(analysisContext.disease)}` : ''}</div></div><button onClick={clearChat} className="ml-auto grid h-10 w-10 place-items-center rounded-xl text-[#879189] hover:bg-white" title="Clear chat"><RotateCcw size={16} /></button></div>
      <div className="min-h-[430px] space-y-5 bg-[#f8faf6] p-5 sm:p-7">{messages.length === 0 && <div className="grid min-h-[300px] place-items-center text-center"><div><div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[#e2f1dd] text-[#4a7d51]"><Leaf /></div><div className="mt-4 font-display font-extrabold">Start a new plant conversation</div><p className="mt-1 text-sm text-[#7d897f]">Choose a prompt below or ask anything.</p></div></div>}{messages.map((message, index) => <div key={index} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[82%] rounded-[22px] px-4 py-3 ${message.role === 'user' ? 'rounded-br-md bg-[#173b27] text-sm leading-6 text-white' : 'rounded-bl-md border border-[#e0e9dc] bg-white shadow-sm'}`}>{message.role === 'assistant' ? <AssistantResponse text={message.text} /> : message.text}</div></div>)}{busy && <div className="flex"><div className="inline-flex items-center gap-2 rounded-[22px] rounded-bl-md border border-[#e0e9dc] bg-white px-4 py-3 text-sm text-[#809087]"><span className="h-3.5 w-3.5 rounded-full border-2 border-[#d7e5d1] border-t-[#5f8c62] animate-spin"/>{loadingText}</div></div>}</div>
      <div className="border-t border-[#e8eee5] p-5"><div className="mb-4 flex flex-wrap gap-2">{suggestions.map(suggestion => <button key={suggestion} onClick={() => void send(suggestion)} disabled={busy} className="rounded-full border border-[#dce7d8] bg-[#f8faf6] px-3.5 py-2 text-xs font-semibold text-[#55705b] transition hover:-translate-y-0.5 hover:bg-[#eef6ea] disabled:opacity-50">{suggestion}</button>)}</div><form onSubmit={event => {event.preventDefault(); void send();}} className="flex gap-2 rounded-2xl border border-[#dce6d9] bg-[#fbfcfa] p-2 focus-within:border-[#98bd90]"><input value={text} onChange={event => setText(event.target.value)} disabled={busy} placeholder="Ask about your plant..." className="min-w-0 flex-1 bg-transparent px-3 text-sm outline-none placeholder:text-[#a0aaa2] disabled:opacity-60" /><button className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-[#173b27] text-white transition hover:bg-[#2a5a3a] disabled:opacity-40" disabled={!text.trim() || busy} title="Send message"><Send size={17} /></button></form></div>
    </div></div>
  </div>;
}
